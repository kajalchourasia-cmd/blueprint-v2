...
def build_controller() -> dict:
    nodes = [
        ...
        code(
            ...
            1360,
            -300,
            r"""const prior=$('Build Scheduler Request').first().json;const cycle=$json.controller_cycle??prior.controller_cycle??0;return [{json:{scheduler_result:$json,authorization:$json.authorization??prior.authorization,run_id:$json.run_id??prior.run_id,project_id:$json.project_id??prior.project_id,profile_version:$json.profile_version??prior.profile_version,idea_text:prior.idea_text,profile:prior.profile,requested_research:prior.requested_research,planning_mode:prior.planning_mode,claim_limit:Math.max(1,Math.min(3,Number(prior.claim_limit??1))),correlation_id:$json.correlation_id??prior.correlation_id,controller_cycle:Number(cycle),rpc_body:{p_run_id:$json.run_id??prior.run_id}}}];""",
        ),

...
> **30 August UX/research correction:** the evaluator workspace now uses a live three-pane section layout with automatic status refresh, explanatory verdict/gate screens, persistent multi-choice onboarding, state-colored processing/completed/blocked navigation, progress-tracked checkable actionables, and collapsed sources/process details. The Stage 1 specialist contract now requires a cited competitor matrix and richer module-specific founder outputs, rejects directories/research tools as competitors, and deduplicates limitations. Existing immutable runs keep their historical output; use the approval-gated rerun flow to regenerate them.

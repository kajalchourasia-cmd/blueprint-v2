...
def _render_kpi_strip(score: float | None, coverage: int, risks: int, completion: int, risk_items: list[str] | None = None) -> None:
    ...
    metrics = (
        ("Decision score", score_value, score_status, score_tone, "Evidence-weighted verdict", "→" if score is None else "↑" if score >= 60 else "↓", "<b>How this is calculated</b>The score appears after Stage 1 reconciles customer, competitor, and market evidence. It is not a probability of success."),
        ("Evidence coverage", f"{coverage}%", coverage_status, coverage_tone, "Accepted research coverage", "↑" if coverage else "→", f"<b>What is covered</b>{coverage}% of the decision-critical research boundary currently has accepted evidence. Rejected or unaudited claims do not increase this value."),
        ("Open risks", str(risks), risk_status, risk_tone, "Items needing resolution", "↓" if risks == 0 else "↑", risk_detail),
        ("Blueprint progress", f"{completion}%", progress_status, progress_tone, "Current roadmap completion", "↑" if completion else "→", f"<b>Roadmap status</b>{completion}% of the currently planned Blueprint modules are complete. Locked future stages do not count as completed work."),
    )
    cards = "".join(
        f'<div class="kpi"><span class="kpi-label">{html.escape(label)}</span><span class="kpi-badge {tone}">{html.escape(status)}</span>'
        f'<div class="kpi-value-row"><b>{html.escape(value)}</b><span class="kpi-trend {tone}">{arrow}</span></div><span class="kpi-meta">{html.escape(meta)}</span><div class="kpi-tooltip">{detail}</div></div>'
        for label, value, status, tone, meta, arrow, detail in metrics
    )

...
.kpi-grid{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0;margin:18px 0 22px;border:1px solid #dde3de;border-radius:14px;background:#fff;box-shadow:0 8px 24px rgba(28,46,34,.045);overflow:hidden}}.kpi{{min-width:0;padding:12px 17px 11px;border-right:1px solid #e6eae6;background:#fff}}.kpi:last-child{{border-right:0}}.kpi-label{{display:block;margin-bottom:5px;color:#747d76;font:500 8px/1.2 'DM Mono';letter-spacing:.015em}}.kpi-value-row{{display:flex;align-items:center;gap:7px;min-height:27px}}.kpi-value-row b{{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#172019;font:600 23px/1 'Space Grotesk';letter-spacing:-.055em}}.kpi-badge{{display:inline-flex;align-items:center;gap:3px;flex:0 0 auto;padding:3px 6px;border-radius:9px;background:#e9f4ec;color:#2f774c;font:500 7px/1 'DM Mono';text-transform:uppercase}}.kpi-badge:before{{content:'↗';font-size:7px}}.kpi-badge.watch{{background:#f8eee6;color:#a15c35}}.kpi-badge.watch:before{{content:'!'}}.kpi-badge.neutral{{background:#eef0ee;color:#737a75}}.kpi-badge.neutral:before{{content:'·'}}.kpi-meta{{display:block;margin-top:5px;color:#a0a6a1;font:7px/1.25 'DM Mono'}}
...
{state_css}@keyframes spin{{to{{transform:rotate(360deg)}}}}@keyframes pulse{{50%{{opacity:.3}}}}@keyframes pop{{50%{{transform:scale(1.25)}}}}@media(max-width:1100px){{.kpi-grid{{grid-template-columns:repeat(2,1fr)}}.kpi:nth-child(2){{border-right:0}}.kpi:nth-child(-n+2){{border-bottom:1px solid #e6eae6}}.st-key-bp_left_rail,.st-key-bp_center_pane,.st-key-bp_right_rail{{height:auto;min-height:0;border-radius:18px}}}}@media(max-width:620px){{.kpi-grid{{grid-template-columns:1fr}}.kpi,.kpi:nth-child(2){{border-right:0;border-bottom:1px solid #e6eae6}}.kpi:last-child{{border-bottom:0}}}}

...
@media(max-width:800px){main .block-container{padding:0 18px 70px}.landing-intro h1{margin-top:25px}.bp-cards{grid-template-columns:1fr}.bp-section{padding:64px 0}.bp-start{display:block;padding:27px}.bp-start h2{font-size:40px}.bp-try{margin-top:20px}.st-key-hero_idea_entry{height:auto;min-height:320px}}
</style>
""",
...
st.markdown(
    """
<section class="bp-section"><div class="bp-rule">02 / The promise</div><h2>Do not build the whole thing. Find the next proof.</h2><div class="bp-cards"><div class="bp-card"><strong>Reality before momentum</strong><span>See the assumptions your idea is quietly asking you to make.</span></div><div class="bp-card"><strong>Evidence-producing work</strong><span>Turn the next unknown into something you can test outside the screen.</span></div><div class="bp-card"><strong>The cost of continuing</strong><span>Know what the next commitment takes from your money, time, and life.</span></div></div></section>
<section class="bp-section"><div class="bp-rule">03 / The method</div><h2>Every next step should earn the one after it.</h2><div class="bp-cards"><div class="bp-card"><strong>01 — Name the bet</strong><span>State what must be true before you invest more.</span></div><div class="bp-card"><strong>02 — Test in the world</strong><span>Capture behavior, commitments, and contradictions.</span></div><div class="bp-card"><strong>03 — Decide honestly</strong><span>Continue, change direction, or stop with the evidence in front of you.</span></div></div></section>
<section class="bp-section" id="start"><div class="bp-start"><div><div class="bp-rule" style="border-color:#555;color:#aeb4af">04 / Begin</div><h2 style="margin-bottom:17px">Bring the unfinished version.</h2><p style="max-width:590px;margin:0;color:#b9bfba;font:15px/1.5 'Space Grotesk',sans-serif">You do not need a pitch deck or polished answers. The field above is enough to begin.</p></div><a class="bp-try" href="#">GO TO THE IDEA FIELD ↑</a></div></section>
""",
    unsafe_allow_html=True,
)
...
def questions_dialog():
    ...
    if q == 0:
        ...
        answers["research_selection"] = st.multiselect(
        ...
        answers["target_customer"] = st.multiselect("Who do you want to build this for?", customer_types, default=answers.get("target_customer", []))
        ...
        answers["prior_work"] = st.multiselect("What have you already done?", prior_work, default=answers.get("prior_work", []))
        answers["current_work"] = st.text_input("Anything important to add?", value=answers.get("current_work", ""), placeholder="Optional detail")
    else:
        answers["constraints"] = st.multiselect("What could constrain you?", contexts, default=answers.get("constraints", []))
